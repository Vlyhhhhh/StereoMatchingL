...
void CrossAggregator::Aggregate(const sint32& num_iters)
{
	...
	//Superpixel segmentation to divide regions
...
void CrossAggregator::FindVerticalArm(const sint32& x, const sint32& y, uint8& top, uint8& bottom) const
{
	...
	for (sint32 k = 0; k < 2; k++) {
		...
		for (sint32 n = 0; n < std::min(cross_L1_, MAX_ARM_LENGTH); n++) {
			...
			//Liu：Superpixel boundary judgment: If the pixel on the current arm is not within the superpixel boundary, exit the loop
			if (!IsInSuperPixelBoundary(slic_left2, width_, x, yn, resultLabel))
			{
				break;
			}
...
//Liu：Determine whether a pixel is within the superpixel boundary
bool CrossAggregator::IsInSuperPixelBoundary(std::vector<uint8> slic_image, int width, int x, int y, Mat  slic_label) const {
	int len = 5; // The size of the superpixel needs to be adjusted according to the actual situation
	int slic_width = width / len; // Width after superpixel segmentation
	int slic_x = x / len; // The x-coordinate of the superpixel region to which the pixel belongs
	int slic_y = y / len; // The y-coordinate of the superpixel region to which the pixel belongs

	// According to the superpixel segmentation result, get the superpixel label value to which the current pixel belongs
	int label = slic_label.at<int>(slic_y, slic_x);

	// If the superpixel label value is 0, it means that the pixel is not within the superpixel boundary.
	// If the superpixel label value is not 0, it means that the pixel is within the superpixel boundary.
	return label != 0;
}
void CrossAggregator::Calculateslic(cv::Mat& img_left, cv::Mat& img_right)
{
	...
	int m = 30;//a parameter witch adjust the weights of spacial distance and the color space distance
	...
	adcensus_util::matToVector(slic_left_2, slic_left2);
	adcensus_util::matToVector(slic_right_2, slic_right2);
}
