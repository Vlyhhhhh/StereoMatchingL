...
SILCStereo::SILCStereo() : width_(0), height_(0), img_left_(nullptr), img_right_(nullptr),
disp_left_(nullptr), disp_right_(nullptr),
is_initialized_(false) { }

SILCStereo::~SILCStereo()
{
	Release();
	is_initialized_ = false;
}

bool SILCStereo::Initialize(const sint32& width, const sint32& height, const SILCStereoOption& option)
{
	// ··· 赋值
...
bool SILCStereo::Match(const uint8* img_left, const uint8* img_right, float32* disp_left)
{
	if (!is_initialized_) {
		return false;
	}
...
bool SILCStereo::Reset(const uint32& width, const uint32& height, const SILCStereoOption& option)
{
	// 释放内存
	Release();
...
void SILCStereo::ComputeCost()
{
	// 设置代价计算器数据
	cost_computer_.SetData(img_left_, img_right_);
	// 设置代价计算器参数
	cost_computer_.SetParams(option_.lambda_ad, option_.lambda_census, option_.lambda_sobel);
	// 计算代价
	cost_computer_.Compute();
}

void SILCStereo::CostAggregation()
{
	// 设置聚合器数据
...
void SILCStereo::ScanlineOptimize()
{
	// 设置优化器数据
...
void SILCStereo::MultiStepRefine()
{
	// 设置多步优化器数据
...
void SILCStereo::ComputeDisparity()
{
...
void SILCStereo::ComputeDisparityRight()
{
	const sint32& min_disparity = option_.min_disparity;
	const sint32& max_disparity = option_.max_disparity;
	const sint32 disp_range = max_disparity - min_disparity;
